...
export class DocumentStateComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.documentStateService
        ...
        .subscribe((res: HttpResponse<IDocumentState[]>) => this.paginateDocumentStates(res.body, res.headers));
      return;
    }
    this.documentStateService
      .query({
  ...
  transition() {
    this.router.navigate(['/document-state'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IDocumentState) {
    return item.id;
  }

  registerChangeInDocumentStates() {
    this.eventSubscriber = this.eventManager.subscribe('documentStateListModification', () => this.loadAll());
  }

  delete(documentState: IDocumentState) {
    const modalRef = this.modalService.open(DocumentStateDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected paginateDocumentStates(data: IDocumentState[], headers: HttpHeaders) {
